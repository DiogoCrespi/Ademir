...
let modalEstorno;
let modalEstornoTitle;
let modalEstornoClose;
let modalEstornoFechar;
let estornoContent;
let cartaoEstorno = null;
let transacoesEstorno = [];
let transacoesSelecionadasEstorno = new Set();
    ...
    try {
      card.innerHTML = `
        ...
        <div class="card-actions">
          ...
          <button class="btn-small" onclick="window.verEstorno && window.verEstorno(${cartao.id})" style="background: #dc2626; color: white; border-color: #dc2626;">Estornar</button>
...
async function verExtrato(id) {
  ...
  // Definir filtros para o dia atual
  const hoje = new Date();
  const dataHoje = hoje.toISOString().split('T')[0]; // Formato YYYY-MM-DD
  
  if (extratoDataInicio) extratoDataInicio.value = dataHoje;
  if (extratoHoraInicio) extratoHoraInicio.value = '00:00';
  if (extratoDataFim) extratoDataFim.value = dataHoje;
  if (extratoHoraFim) extratoHoraFim.value = '23:59';
...
function inicializarElementos() {
  ...
  extratoDataFim = document.getElementById('extratoDataFim');
  extratoHoraFim = document.getElementById('extratoHoraFim');
  modalEstorno = document.getElementById('modalEstorno');
  modalEstornoTitle = document.getElementById('modalEstornoTitle');
  modalEstornoClose = document.getElementById('modalEstornoClose');
  modalEstornoFechar = document.getElementById('modalEstornoFechar');
  estornoContent = document.getElementById('estornoContent');
...
function configurarEventListeners() {
  ...
  if (modalEstornoClose) {
    modalEstornoClose.addEventListener('click', fecharModalEstorno);
  }
  if (modalEstornoFechar) {
    modalEstornoFechar.addEventListener('click', fecharModalEstorno);
  }
  if (searchInput) {
  ...
  if (modalEstorno) {
    modalEstorno.addEventListener('click', (e) => {
      if (e.target === modalEstorno) fecharModalEstorno();
    });
  }
...
// Ver estorno do cartão
async function verEstorno(id) {
  console.log('[Admin] verEstorno chamado com id:', id);
  ...
  cartaoEstorno = cartao;
  transacoesSelecionadasEstorno.clear();
  ...
  if (!modalEstorno || !modalEstornoTitle || !estornoContent) {
    showToast('Erro ao abrir estorno. Recarregue a página.');
    return;
  }
  
  modalEstornoTitle.textContent = `Estornar Transações - Cartão ${cartao.numero} - ${cartao.nome}`;
  estornoContent.innerHTML = '<div class="empty-state">Carregando transações...</div>';
  modalEstorno.classList.add('open');
  
  try {
    const todasTransacoes = await CartoesAPI.getTransacoes(cartao.id);
    // Filtrar apenas compras não estornadas
    transacoesEstorno = todasTransacoes.filter(t => t.tipo === 'compra' && !t.estornado);
    console.log('[Admin] Transações para estorno:', transacoesEstorno.length);
    renderEstorno();
  } catch (error) {
    console.error('[Admin] Erro ao carregar transações:', error);
    estornoContent.innerHTML = '<div class="empty-state">Erro ao carregar transações. Tente novamente.</div>';
  }
}

// Renderizar lista de transações para estorno
function renderEstorno() {
  if (!estornoContent || !cartaoEstorno) return;
  
  if (transacoesEstorno.length === 0) {
    estornoContent.innerHTML = '<div class="empty-state">Nenhuma transação de compra disponível para estorno</div>';
  ...
  transacoesEstorno.forEach(trans => {
    const data = new Date(trans.data);
    const dataFormatada = data.toLocaleDateString('pt-BR');
    const horaFormatada = data.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });
    const isSelected = transacoesSelecionadasEstorno.has(trans.id);
    const itensHtml = trans.itens && trans.itens.length > 0 
      ? trans.itens.map(item => `${item.quantidade}x ${item.produtoNome || item.nome || 'Item'}`).join(', ')
      : 'Sem detalhes';
    
    html += `
      <div style="padding: 12px; background: ${isSelected ? '#1a2332' : '#0d1423'}; border: 2px solid ${isSelected ? 'var(--accent)' : '#1f2937'}; border-radius: 8px; cursor: pointer; transition: all 0.2s;" 
           onclick="toggleTransacaoEstorno(${trans.id})">
        <div style="display: flex; align-items: start; gap: 12px;">
          <input type="checkbox" ${isSelected ? 'checked' : ''} 
                 onclick="event.stopPropagation(); toggleTransacaoEstorno(${trans.id})" 
            ...
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 8px;">
              ...
              <div style="text-align: right;">
                <div style="font-size: 18px; font-weight: 700; color: var(--danger);">
                  ${formatPrice(trans.valor)}
                </div>
                <div style="font-size: 11px; color: var(--muted);">
                  Saldo: ${formatPrice(trans.saldoAtual)}
                </div>
              </div>
            </div>
            ${trans.descricao ? `<div style="font-size: 11px; color: var(--muted); margin-top: 4px;">${trans.descricao}</div>` : ''}
          </div>
        </div>
      </div>
    `;
  ...
  html += '</div>';
  estornoContent.innerHTML = html;
  atualizarTotalSelecionado();
}

// Toggle seleção de transação
function toggleTransacaoEstorno(transacaoId) {
  if (transacoesSelecionadasEstorno.has(transacaoId)) {
    transacoesSelecionadasEstorno.delete(transacaoId);
  } else {
    transacoesSelecionadasEstorno.add(transacaoId);
  }
  renderEstorno();
}

// Selecionar todas as transações
function selecionarTodasEstorno() {
  transacoesEstorno.forEach(t => transacoesSelecionadasEstorno.add(t.id));
  renderEstorno();
}

// Desmarcar todas as transações
function desmarcarTodasEstorno() {
  transacoesSelecionadasEstorno.clear();
  renderEstorno();
}

// Atualizar total selecionado
function atualizarTotalSelecionado() {
  const totalElement = document.getElementById('estornoTotalSelecionado');
  if (!totalElement) return;
  
  const total = Array.from(transacoesSelecionadasEstorno).reduce((sum, transId) => {
    const trans = transacoesEstorno.find(t => t.id === transId);
    return sum + (trans ? (parseFloat(trans.valor) || 0) : 0);
  }, 0);
...
async function confirmarEstorno() {
  if (transacoesSelecionadasEstorno.size === 0) {
    showToast('Selecione pelo menos uma transação para estornar');
    return;
  }
  
  const transacoesIds = Array.from(transacoesSelecionadasEstorno);
  const total = transacoesIds.reduce((sum, transId) => {
    const trans = transacoesEstorno.find(t => t.id === transId);
    return sum + (trans ? (parseFloat(trans.valor) || 0) : 0);
  }, 0);
  
  if (!confirm(`Confirmar estorno de ${transacoesIds.length} transação(ões) no valor total de ${formatPrice(total)}?`)) {
    return;
  }
  
  try {
    const resultado = await CartoesAPI.estornar(cartaoEstorno.id, transacoesIds);
    if (resultado.success) {
      showToast(`Estorno realizado com sucesso! Valor: ${formatPrice(resultado.valorEstornado)}`);
      
      // Atualizar saldo do cartão na lista
      const cartaoIndex = cartoes.findIndex(c => c.id === cartaoEstorno.id);
      if (cartaoIndex !== -1) {
        cartoes[cartaoIndex].saldo = resultado.cartao.saldo;
        saldosAnteriores[cartaoEstorno.id] = resultado.cartao.saldo;
      }
      ...
      showToast('Erro ao processar estorno: ' + (resultado.message || 'Erro desconhecido'));
    }
  } catch (error) {
    console.error('[Admin] Erro ao processar estorno:', error);
    showToast('Erro ao processar estorno. Tente novamente.');
  }
}

// Fechar modal de estorno
function fecharModalEstorno() {
  if (modalEstorno) {
    modalEstorno.classList.remove('open');
  }
  cartaoEstorno = null;
  transacoesEstorno = [];
  transacoesSelecionadasEstorno.clear();
...
window.verEstorno = verEstorno;
window.toggleTransacaoEstorno = toggleTransacaoEstorno;
window.selecionarTodasEstorno = selecionarTodasEstorno;
window.desmarcarTodasEstorno = desmarcarTodasEstorno;
